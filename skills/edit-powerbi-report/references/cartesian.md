...
Use the role and formatting-object names documented in this file. For reference,
the `lineChart` roles are:
...
> **Property names vary by visual type.** Use the property names documented in
> the sections below; some apply only to specific visual types as noted.
...
> on bar/column vs `transparency` on lineChart).
...
point instances. `Function: 3` is Min aggregation.
...
(e.g., `stackedGapSize` instead of `clusteredGapSize`).
...
metadata selectors for per-series styling.
...
Labels at the end of each line series (lineChart only).
...
> **Note:** All `y2Axis` properties are prefixed with `sec`.
