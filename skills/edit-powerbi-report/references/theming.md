...
> with the same name can leave stale theme state. Changing the filename on every
> update forces a fresh load when the PBIP is next opened in Power BI Desktop.
...
> Using the bare theme name (e.g., `"MyTheme-a1b2c3d4"`) causes the theme to be
> applied incorrectly because the resource mapping never matches the file under
...
Worked examples: `#118DFF` at `0.4` → `#70BBFF` (lighter); `#118DFF` at `-0.4`
→ `#0A5599` (darker).
...
5. **Reference a schema** — see [Schema Version](#schema-version) above for
   how to pick and reference the right `reportThemeSchema-<version>.json`. For
   schema validation in an editor, set `"$schema"` to a full published HTTPS URL
   or a local schema file beside the theme JSON.
