...
> recalculate height when changing mode. Watch for any slicer whose
> `position.height` is below the per-mode floor — it will clip the bottom row.
...
Common formatting objects per slicer type:

- `slicer` — `data`, `header`, `items`, `selection`, `general`
- `listSlicer` — `header`, `items`, `selection`
- `advancedSlicerVisual` — `header`, `items`, `selection` (uses `id`-selector
  states: `default`, `hover`, `press`, `selected`, `mixed`)

The `header` object controls the header label (`show`, `text`, font); `items`
controls the item rows (font, `padding`); `data` (classic `slicer` only)
controls `mode`; `general.filter` controls value selections.
