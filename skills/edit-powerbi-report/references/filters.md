...
uses `"Entity"`. An entity ref that slips into `Where` is a common error. To
inventory every filter defined in a report (report / page / visual scopes),
search the `definition/` tree for `filterConfig` blocks across `report.json`,
`page.json`, and `visual.json`, then open each file to inspect the actual
