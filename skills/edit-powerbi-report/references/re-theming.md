...
> **Complete this entire step before reviewing the result.**
...
Properties that hold a `solid.color` fill are the ones that need color
adjustment — see the per-visual-type reference linked in the table above for
the full list of color objects on each visual type.
