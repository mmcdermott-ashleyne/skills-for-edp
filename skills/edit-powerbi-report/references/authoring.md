...
> **Before editing existing reports:** inventory the report by browsing the
> `definition/pages/` directory — each page has a `page.json` and a `visuals/`
> subfolder with one `visual.json` per visual.
>
> **Examples are structural patterns.** Property names may vary by visual type.
> Use the property and object names documented in the per-visual references
> (see [Visual Type References](#visual-type-references)) when applying formatting.
...
| Filter `name` | `Filter` + 24 lowercase hex chars (e.g. `Filter1a2b3c4d5e6f7890a1b2c3d4`) | Unique across the entire report definition — duplicate filter names cause load errors |
...
— rotate the GUID suffix (keep `<CustomThemeName>` stable) and update all
`report.json` references. Only change `<CustomThemeName>` if the user
...
> the bare theme name causes the theme to apply incorrectly. The `name` field
> **inside the theme JSON file itself** stays as the bare theme name (no
> `.json` suffix). See `theming.md` for details.
