...
Common `shape` objects are `shape` (with the `tileShape` enum: `rectangle`,
`arrow`, etc.), `fill`, `outline`, `rotation`, `text`, `shadow`, and `glow`,
plus the shape-specific geometry parameters shown in the examples on this page.
...
All shape objects listed above require the `{ "id": "default" }` selector except
`rotation` (see below).
...
Reserve shape text for simple labels only. When shape text is used, always set
an explicit `fontColor` and verify contrast against the rendered fill/background.
