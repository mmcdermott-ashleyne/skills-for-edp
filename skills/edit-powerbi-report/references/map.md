...
must never be created (they are deprecated visual types).
...
3. **Ask the user for clarification** — if you cannot resolve the geocoding issue,
   describe the problem and ask which field to use or whether lat/lon columns
   are available
4. **Do not silently substitute a non-map visual** for data/geocoding issues — if
   ...
   ask the user before changing visual types. Substituting a non-map visual for a
   resolvable data problem violates the user's intent.
5. **When Azure Maps is unavailable in the environment** (e.g., disabled by tenant
   ...
   `filledMap`, and `shapeMap` visuals as fallbacks.
