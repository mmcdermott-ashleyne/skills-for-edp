...
corner radius, effects). The key properties are listed in the table below.
...
concerns (border, background, corner radius), use the table above to choose the
correct object before setting a value. The `image` object properties control the
image content area; VCO properties control the outer card container.
...
The `plotArea` object is available on cartesian chart visuals (bar, column,
line, area, etc.). Use the nested `image.image` sub-structure shown below.
