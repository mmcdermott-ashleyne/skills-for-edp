...
> **⚠️ Property names and enum values vary between visual types.**
> Patterns here show structure; use the property names documented in this file
> and the visual-type-specific reference files for the exact names.
...
their formatting objects to take effect — see the table below for which objects
need selectors.
...
> ⚠️ **Renders without the right selector?** Some properties produce no visual
> effect without the correct selector. This is the hardest class of bug to
> debug. If formatting has no effect, check whether the object requires an `id`
> selector (see the table above) and apply the dual-entry pattern.
...
> ⚠️ If the visual ends up with duplicate titles (your custom title + an
...
Do not mix page and VCO property lists — page objects and VCOs share names but
have different valid properties.
