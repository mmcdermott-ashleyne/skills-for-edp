...
After each logical batch of changes, review the diff before committing:

```bash
git -C "<pbip-folder>" diff
```

After editing, the user opens the PBIP in Power BI Desktop to confirm rendering.

### 4. Ask user before committing

**Never auto-commit.** After the changes are reviewed, ask the
...
- **More changes requested**: Continue editing on the same branch, review the
  diff after each batch, ask user before each commit.
...
Create working branch → copilot/<intent>
    │
    ▼
Make changes → Review diff → Commit
    │
    ├── Edit went wrong → Revert uncommitted (git checkout -- .)
    │                     → Fix and retry
    │
